...
def get_data_training(
    DRIVE_train_imgs_original_hdf_fp,
    DRIVE_train_groudTruth_hdf_fp,
    patch_height,
    patch_width,
    N_subimgs,
    inside_FOV,
    ...
    train_imgs_original = load_hdf5(DRIVE_train_imgs_original_hdf_fp)
    train_masks = load_hdf5(DRIVE_train_groudTruth_hdf_fp) #masks always the same
    # visualize(group_images(train_imgs_original[0:20,:,:,:],5),'imgs_train')#.show()  #check original imgs train

    # preprocess images
    if verbose:
        print("preprocessing on training images...")
    train_imgs = preprocess_retina_imgs(train_imgs_original)
    ...
    if verbose:
        print(f"\ntraining images/masks info:")
        print(40*'-')
        print(f"  data shape for both imgs/msks:", train_imgs.shape)
        print(f"  images range (min-max): {np.min(train_imgs)} - {np.max(train_imgs)}")
        print(f"  masks range (min-max): {np.min(train_masks)} - {np.max(train_masks)}")
    ...
    if verbose:
        print(f"  data shape for both imgs_pathces/msks_pathces):", patches_imgs_train.shape)
        print(f"  imgs_pathces range (min-max): {np.min(patches_imgs_train)} - {np.max(patches_imgs_train)}")
...
def get_data_testing(
    DRIVE_test_imgs_original_hdf_fp, 
    DRIVE_test_groudTruth_hdf_fp, 
    Imgs_to_test, 
    patch_height, 
    patch_width,
    verbose = True
    ...
    ### test
    test_imgs_original = load_hdf5(DRIVE_test_imgs_original_hdf_fp)
    test_masks = load_hdf5(DRIVE_test_groudTruth_hdf_fp)

    # preprocess images
    if verbose:
        print("preprocessing on test images...")
    test_imgs = preprocess_retina_imgs(test_imgs_original)
    ...
    if verbose:
        print(f"\ntest images/masks info:")
        print(40*'-')
        print(f"  data shape for both imgs/msks:", test_imgs.shape)
        print(f"  images range (min-max): {np.min(test_imgs)} - {np.max(test_imgs)}")
        print(f"  masks range (min-max): {np.min(test_masks)} - {np.max(test_masks)}")
    ...
    if verbose:
        print(f"  data shape for both imgs_pathces/msks_pathces):", patches_imgs_test.shape)
        print(f"  imgs_pathces range (min-max): {np.min(patches_imgs_test)} - {np.max(patches_imgs_test)}")
...
def get_data_testing_overlap(
    ...
    if verbose:
        print(f"\ntest images/masks info:")
        print(40*'-')
        print(f"  data shape for images:", test_imgs.shape)
        print(f"  data shape for masks:", test_masks.shape)
        print(f"  images range (min-max): {np.min(test_imgs)} - {np.max(test_imgs)}")
        print(f"  masks range (min-max): {np.min(test_masks)} - {np.max(test_masks)}")
